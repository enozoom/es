...
class Upload{
  ...
  protected $error_messages = array(
      1 => '上传的文件超过了php.ini中设置的upload_max_filesize',
      2 => '上传的文件超过表单中指定的max_file_size',
      3 => '仅有部分文件被上传',
      4 => '文件未被上传',
      6 => '文件上传的临时文件夹未找到',
      7 => '文件上传时写入硬盘失败',
      ...
      'max_file_size' => '文件超过要求大小',
      'min_file_size' => '文件小于要求大小',
      'accept_file_types' => '被禁止的文件类型',
      'max_number_of_files' => '超出最大上传数',
      'max_width' => '图片超过要求宽度',
      'min_width' => '图片需有最低宽度',
      'max_height' => '图片超过要求高度',
      'min_height' => '图片须有最低高度',
  ...
  public function initialize($options = null, $error_messages = null) {
    if ($options) {
      $this->options = $options + $this->options;
      if(!empty($options['dir'])){
        $this->options['upload_dir'] .= $options['dir'].date('/Y/m/');
        $this->options['upload_url'] .= $options['dir'].date('/Y/m/');
      }
    }
    \es\core\mk_dir( substr($this->options['upload_dir'],0,-1) );
    if ($error_messages) {
