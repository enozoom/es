...
*/  
  public function _get($tablename,$where='',$select='*',$orderby=FALSE,$limit=''){
    ...
    empty($select) && $select = '*';
    $select == '*' || $select = '`'.str_replace(',','`,`', \es\core\clean_wordblank($select)).'`';
 ...
 */
  protected function log_error(){
   ...
   \es\core\log_msg($this->mysqli->error.PHP_EOL.'详情：'.PHP_EOL.$trace,'MYSQL数据异常');
  }
}