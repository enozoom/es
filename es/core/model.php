...
class Model{
  ...
  public function __construct(){
    ...
    $this->encryption_key = $configs->config->encryption_key;
  }
/**
 * PHP5内置函数
 * @param unknown $var
...
*/  
  public function _get($where='', $select='*', $orderby='', $limit=[]){
    if(!empty($limit)){
       $per = $limit[0];
       if(count($limit)==1){
        $limit = $per;
...
* @return Array(Object)
*/
  public function _get_with_relation($where='', $select='*',$orderby='',$limit=[]){
    $rel = [];
    $rals = $this->_relation();
    if(!empty($rals)){
...
* @return string
*/  
  protected function generate_password($pword){
    return md5('ENOZOOM'.$this->encryption_key.sha1($pword));
  }
/**
* 过滤非法字符
* @param array $post
